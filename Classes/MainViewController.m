...
- (void)viewDidLoad {
    ...
    if(wifiBoolean == YES){
        ...
        UIAlertView *myAlertView = [[UIAlertView alloc] initWithTitle:@"No internet" message:@"MAIN VIEW:Please check that an internet connection is available" delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil];
        [myAlertView show];
...
-(BOOL) checkForInternet{
  ...
  // reach = [Reachability reachabilityWithHostname:kDynamicMapServiceURL];
    
    // this instead?
    reach = [Reachability reachabilityWithHostname:@"http://www.google.com"];
    
    // this is to add a test for a hardware problem instead of pointing to a URL
    // not implemented yet
   ...
   // internetReachable = [Reachability reachabilityForInternetConnection];
    ...
    reach.reachableBlock = ^(Reachability * reachability)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"initial check wifi available");
   ...
   // NSLog(@"responseBoolean: %i", responseBoolean);
    NSLog(@"connection? = %s", responseBoolean ? "YES" : "NO");
